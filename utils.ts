...
/**
 * Generates a WhatsApp link that attempts to trigger the installed application.
 * Uses the whatsapp:// protocol on mobile and api.whatsapp.com on desktop.
 */
export const getWhatsAppUrl = (phone: string, message: string): string => {
  ...
  // Detect mobile to use the direct protocol which forces the app
  const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

  if (isMobile) {
    // This protocol directly targets the installed mobile app
    return `whatsapp://send?phone=${finalPhone}&text=${encodedMessage}`;
  }

  // For desktop, api.whatsapp.com is more reliable than wa.me for triggering the desktop app prompt
